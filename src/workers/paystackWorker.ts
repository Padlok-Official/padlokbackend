...
export const setupPaystackWorker = () => {
  const worker = new Worker(
    ...
    async (job: Job<PaystackWebhookEvent>) => {
      ...
      console.log(`Processing Paystack event: ${event.event} [Job ID: ${job.id}]`);
...
async function handleChargeSuccess(event: PaystackWebhookEvent): Promise<void> {
  ...
  if (verified.status !== 'success') {
    throw new Error(`Transaction ${reference} verification failed: status is ${verified.status}`);
  }
  ...
  if (transaction && transaction.type === 'deposit') {
    ...
    if (!walletId) throw new Error(`No wallet_id in transaction metadata for ${reference}`);
