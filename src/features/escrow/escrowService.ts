...
export const escrowService = {
  async initiateEscrow(params: {
    ...
    if (seller.id === buyerId)
      throw new AppError("You cannot initiate an escrow with yourself", 400);
    ...
    socketService.emitToUser(buyerId, "transaction:updated", {
      id: escrowTx.id,
    });
  ...
  async setDeliveryAndFund(params: {
    userId: string;
    escrowId: string;
    ...
    if (!DELIVERY_HOURS_ALLOWED.includes(deliveryHours)) {
      throw new AppError(
        `delivery_hours must be one of: ${DELIVERY_HOURS_ALLOWED.join(", ")}`,
        400,
      );
    }
    ...
    const result = await withTransaction(async (client) => {
      ...
      if (escrowTx.receiver_id !== userId)
        throw new AppError("Only the seller can set the delivery window", 403);
      if (escrowTx.status !== "initiated")
        throw new AppError(
          `Cannot set delivery. Current status: ${escrowTx.status}`,
          400,
        );
      ...
      if (!buyerWalletId || !sellerWalletId)
        throw new AppError("Wallet information not found", 500);
      ...
      if (!buyerWallet || parseFloat(buyerWallet.balance) < totalAmount) {
        throw new AppError(
          "Buyer has insufficient wallet balance to fund this escrow",
          400,
        );
      }
      ...
      const deliveryDeadline = new Date(
        Date.now() + deliveryHours * 60 * 60 * 1000,
      );
    ...
    await AuditLogModel.log({
      ...
      details: {
        ...
        delivery_deadline: result.deliveryDeadline.toISOString(),
        ...meta,
      },
      ...meta,
    });
  ...
  async confirmDelivery(params: {
    ...
    const escrowTx = await withTransaction(async (client) => {
      const tx = await EscrowTransactionModel.findByIdForUpdate(
        client,
        escrowId,
      );
      if (!tx) throw new AppError("Escrow transaction not found", 404);
      if (tx.receiver_id !== userId)
        throw new AppError("Only the seller can confirm delivery", 403);
      if (tx.status !== "funded")
        throw new AppError(
          `Cannot confirm delivery. Current status: ${tx.status}`,
          400,
        );
  ...
  async confirmReceipt(params: {
    ...
    const escrowTx = await withTransaction(async (client) => {
      ...
      if (tx.user_id !== userId)
        throw new AppError("Only the buyer can confirm receipt", 403);
      if (tx.status !== "delivery_confirmed")
        throw new AppError(
          `Cannot confirm receipt. Current status: ${tx.status}`,
          400,
        );

      const sellerWalletId = (tx.metadata as any)?.receiver_wallet_id;
      if (!sellerWalletId)
        throw new AppError("Seller wallet ID not found in metadata", 500);
      ...
      await EscrowTransactionModel.updateStatus(client, tx.id, "completed", {
      ...
      if (buyerWalletId)
        await WalletModel.debitEscrow(client, buyerWalletId, tx.amount);
  ...
  async raiseDispute(params: {
    ...
    if (!escrowTx) throw new AppError("Escrow transaction not found", 404);
    if (escrowTx.user_id !== userId)
      throw new AppError("Only the buyer can raise a dispute", 403);
    if (!["funded", "delivery_confirmed"].includes(escrowTx.status)) {
      throw new AppError(
        `Cannot dispute. Current status: ${escrowTx.status}`,
        400,
      );
    }
    ...
    const otherUserId =
      userId === escrowTx.user_id ? escrowTx.receiver_id : escrowTx.user_id;
  ...
  async cancelEscrow(params: { userId: string; escrowId: string; meta: Meta }) {
    ...
    if (!escrowTx) throw new AppError("Escrow transaction not found", 404);
    if (escrowTx.user_id !== userId)
      throw new AppError("Only the buyer can cancel", 403);
    if (escrowTx.status !== "initiated") {
      throw new AppError(
        `Cannot cancel. Current status: ${escrowTx.status}. Funds are already locked.`,
        400,
      );
    }

    await withTransaction((client) =>
      EscrowTransactionModel.updateStatus(client, escrowTx.id, "cancelled"),
    );

    const sellerId = escrowTx.receiver_id;
    socketService.emitToUser(sellerId, "escrow:cancelled", {
      id: escrowId,
      reference: escrowTx.reference,
  ...
  async resolveDispute(params: {
    ...
    if (!["open", "under_review"].includes(dispute.status)) {
      throw new AppError(
        `Dispute already resolved with status: ${dispute.status}`,
        400,
      );
    }
    ...
    if (!escrowTx)
      throw new AppError("Associated escrow transaction not found", 500);
    ...
    await withTransaction(async (client) => {
      ...
      if (resolution === "refund") {
        if (!buyerWalletId)
          throw new AppError("Buyer wallet ID not found in metadata", 500);
    ...
    for (const [id, msg] of [
      [
        buyerId,
        `Dispute resolved. Funds ${resolution === "refund" ? "refunded to you" : "released to seller"}.`,
      ],
      [
        sellerId,
        `Dispute resolved. Funds ${resolution === "refund" ? "refunded to buyer" : "released to you"}.`,
      ],
    ] as [string, string][]) {
      socketService.emitToUser(id, "escrow:dispute_resolved", {
        id: escrowTx.id,
        resolution,
        message: msg,
      });
  ...
  async getEscrowById(userId: string, escrowId: string) {
    const escrowTx = await EscrowTransactionModel.findById(escrowId);
    if (!escrowTx) throw new AppError("Escrow transaction not found", 404);
    if (escrowTx.user_id !== userId && escrowTx.receiver_id !== userId)
  ...
  async uploadItemImages(images: string[]) {
    ...
    const results = await Promise.all(
      images.map((img) => cloudinaryService.uploadImage(img, "escrow_items")),
    );
